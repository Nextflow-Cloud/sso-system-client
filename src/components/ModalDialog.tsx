            ...
            <div className="rounded-md bg-white p-5 drop-shadow-lg opacity-100">
                <div className="flex flex-col space-y-4">
                    ...
                    <div className="flex flex-row justify-end">
                        {buttons.map(button => (
                            <button key={button.id} className={`px-4 py-2 mx-2 rounded-md ${button.primary ? "bg-gray-300 hover:bg-gray-400" : "bg-green-500 hover:bg-green-600"}`} onClick={() => onClose(button.id, checkboxRef.current?.checked)}>
                                {button.text}
                            </button>
                        ))}
                    </div>
